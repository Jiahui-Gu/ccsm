...
> - **R4 P1 shutdown admin-only** — `packages/daemon/test/integration/supervisor/admin-only.spec.ts` asserts (a) `shutdown` RPC from a non-admin peer-cred uid is rejected with `PERMISSION_DENIED`, (b) `shutdown` RPC from admin uid (per [03](./03-listeners-and-transport.md) §7.1 uid-allowlist) succeeds and triggers the OS-initiated shutdown path, (c) on Windows the named-pipe DACL denies the connect outright when the caller is not in the local Administrators group.
