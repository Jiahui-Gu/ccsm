...
**DSN topology — locked v0.4 (updated 2026-05-01 phase-3 PR)**

> **Per-surface DSN topology (locked v0.4):** ccsm ships THREE Sentry projects,
> one per surface (renderer, main, daemon), each with its own DSN injected at
> build time via `CCSM_SENTRY_DSN_RENDERER` / `CCSM_SENTRY_DSN_MAIN` /
> `CCSM_SENTRY_DSN_DAEMON`. Rationale: per-surface project isolation gives
> clean separation of release health, alert routing, and quota allocation per
> process boundary; consolidation into a single project would require
> tag-based filtering on every alert rule, which is brittle. The `surface`
> tag is still set on every event for cross-surface correlation in incident
> response.
...
- Implementation:
  - `webpack.DefinePlugin({ 'process.env.SENTRY_DSN_RENDERER': JSON.stringify(process.env.CCSM_SENTRY_DSN_RENDERER) })`
    for the renderer (per-surface key).
  - `scripts/before-pack.cjs` writes `dist/electron/build-info.js` with
    `{ sentryDsnRenderer, sentryDsnMain, sentryDsnDaemon }`. Main-proc init
    reads `process.env.SENTRY_DSN_MAIN ?? process.env.SENTRY_DSN ?? buildInfo.sentryDsnMain`;
    supervisor reads `sentryDsnDaemon` and forwards it as
    `CCSM_DAEMON_SENTRY_DSN` to the daemon child at spawn.
- Release CI uses three secrets — `CCSM_SENTRY_DSN_RENDERER`,
  `CCSM_SENTRY_DSN_MAIN`, `CCSM_SENTRY_DSN_DAEMON` — under the `release`
  environment. PR / fork builds run the same pipeline with the secrets
  absent → DSNs baked in as `''` → Sentry stays off exactly as today.
  Zero risk of a fork accidentally shipping our DSNs.

**OSS-fork leak prevention**

- The DSNs are only ever populated when a workflow on the canonical repo runs
  with the `release` environment that holds the secrets. PRs from forks
  cannot read environment secrets per GitHub's default policy.
- A unit test asserts: when `process.env.SENTRY_DSN_*` is undefined and the
  build-info DSN looks like `''` or a `***REDACTED***` placeholder,
  `initSentry()` (and the daemon equivalent) must short-circuit (already
  does — keep regression test).

**Symbol upload**

- New script `scripts/sentry-upload-symbols.cjs` invoked from
  `electron-builder`'s `afterAllArtifactBuild` hook AND as a release-CI step.
  Uses `@sentry/cli` with `SENTRY_AUTH_TOKEN` + `SENTRY_ORG` +
  `SENTRY_PROJECT_RENDERER` / `SENTRY_PROJECT_MAIN` / `SENTRY_PROJECT_DAEMON`.
  Idempotent + version-keyed.
- Uploads:
  - Renderer source maps + JS bundles from `dist/renderer/` →
    `SENTRY_PROJECT_RENDERER`.
  - Main source maps + JS bundles from `dist/electron/` →
    `SENTRY_PROJECT_MAIN`.
  - Daemon source maps + JS bundles from `daemon/dist/` →
    `SENTRY_PROJECT_DAEMON`.
  - Native debug-info (dSYM on macOS, .pdb on Windows) from
    electron-builder artifact directories → `SENTRY_PROJECT_MAIN` (Crashpad
    symbolication is owned by the main project).
...
- 2026-05-01 phase-3 PR: §6 updated to 3-DSN topology to match implementation; was previously single-DSN aspirational.
