...
- **Layout + schema source of truth**: see `docs/superpowers/specs/2026-05-01-crash-observability-design.md` §10 for the per-incident directory layout (`<dataRoot>/crashes/<YYYY-MM-DD-HHMM>-<incidentId>/` with `meta.json`, `frontend.dmp`, `frontend.log`, `backend.dmp`, `backend.log`, `daemon-marker.json`, `stderr-tail.txt`, `stdout-tail.txt`, `screenshot.png`, `README.txt`) and §5.2 / §9 for the daemon-side `<runtimeRoot>/crash/<bootNonce>.json` marker schema. The crash observability design doc is the single canonical source; this fragment defers all schema details to it.
- **Superseded**: the older single-file schema `<dataRoot>/crashes/crash-<isoTs>-<bootNonce>.json` (daemon) and `electron-crash-<isoTs>-<bootNonce>.json` (Electron-main) with payload `{ side, v, pid, boot, errMessage, errStack, lastTraceIds, lastSessions, stats, ts }` is **superseded by the per-incident directory layout**. Reason: a per-incident dir provides (a) cross-side correlation (`frontend.*` + `backend.*` files filed under one incident `traceId`), (b) native dump (`.dmp`) adoption with Crashpad symbolication, (c) clean separation of `meta.json` (structured metadata), `daemon-marker.json` (uncaught-exception payload), and `stderr-tail.txt` / `stdout-tail.txt` (raw streams) that the single-JSON schema could not represent.
- Path: `<dataRoot>/crashes/` per frag-11 §11.6 paths table (`%LOCALAPPDATA%\ccsm\crashes\` Win, `~/Library/Application Support/ccsm/crashes/` mac, `~/.local/share/ccsm/crashes/` Linux). User-only ACL (0600 / equivalent Win DACL). **Filename components are sanitized via `path.basename(s).replace(/[^A-Za-z0-9._-]/g, '_')` before path concatenation** (r3-sec P1-5 — defense in depth against future ULID-impl swap that emits non-Crockford characters).
- Redaction: every payload (`meta.json`, `daemon-marker.json`, `*.log`) MUST go through the same pino redact list before write — crash dumps must never contain secrets. See crash design doc §7 (PII + consent) for the scrub/allowlist contract.
- Retention: 30 days AND aggregate per-side cap 200 MB (r3-resource P1 — see §6.6.1 aggregate cap rule). Pruning runs (a) at `daemonSupervisor.start()` AND (b) once per pino-roll rotation tick (r3-sec P1-3 — covers the case where daemon never restarts because auto-update is OFF default). Crash design doc §10 also specifies "last 20 incidents OR 30 days, whichever is larger" at electron-main boot — the two rules co-apply (whichever prunes more aggressively wins).
- **NEVER auto-uploaded.** User-driven attach to issue only. (r2-sec T13 row.) Network upload (Sentry envelope) is gated by the `crashReportingOptOut` preference per crash design doc §7.
