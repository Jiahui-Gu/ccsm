...
| Electron `41.3.0` exact pin (no caret/tilde; T61/PR #704 escalated from `^41.3.0` to exact; drift enforced by `scripts/check-electron-pin.cjs`; Electron 42+ deferred post-v0.3) | frag-11 §11.5(0) (E add — toolchain pin owner) | frag-11 §11.2.1 SDK packaging (Electron 41 main = CJS, SDK = ESM); frag-3.7 §3.7.7.b SDK ownership lock; frag-3.5.1 `loadSdk()` shim contract; per-`.node` ABI rebuild matrix | `[manager r10 lock: T61/PR #704 ESCALATE → exact pin 41.3.0; supersedes r7 caret lock]` (frag-11 §11.5(0)) | CLOSED |
