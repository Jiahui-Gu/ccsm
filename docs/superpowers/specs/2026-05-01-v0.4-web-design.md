...
- Blockers: T03.
- **Verdict (2026-05-01, task #1078):** NO-GO direct (blocked by `yao-pkg/pkg#215`, exports-subpath resolver bug); GO via `esbuild → CJS bundle → pkg` two-stage pipeline. End-to-end verified on Win 11; cross-target pkg builds for macOS/Linux produced (CI matrix legs verify natively). T05 unblocked. See spike report for fallback wiring plan.
