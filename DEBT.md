...
| 22 | Signaling relay (`offer`/`answer`/`ice`) carries no nonce / sequence / freshness check, so a duplicate SDP/ICE frame is forwarded as-is. **WONTFIX (accepted risk):** the relay is reached only after a valid `session` JWT **and** `claims.userHash === userHashFromPath` (`doProxy.ts` → 401/403), and the Durable Object is keyed by `userHash` — so a room only ever contains the **same GitHub user's own devices** (cross-user is already prevented). Messages are relayed live in-memory between two open WebSockets and never persisted, so there is no stored-message replay surface; an injector would need a live authenticated same-user WS in the same room. At the WebRTC layer a replayed ICE candidate is idempotent and a stale SDP is rejected by ufrag / m-line checks in werift/the browser. Adding nonce/seq state to an ephemeral, same-user-only relay adds complexity without closing a real exploit path. Revisit only if the room threat model ever widens beyond same-user. | WONTFIX | S | LOW | `cloudflare/src/pairingDo.ts:90-94` |
...
- **2026-05-31 (mobile-remote hardening audit)** — parallel best-practice sweep of the public-internet mobile-remote path queued four findings (#68–#71). #68 (desktop OAuth loopback redirect tightening) → [#1465](https://github.com/Jiahui-Gu/ccsm/pull/1465). #69 split into two independent same-user hardening fixes, both in [#1466](https://github.com/Jiahui-Gu/ccsm/pull/1466): (a) route each relayed ICE candidate to only the `RTCPeerConnection` that owns its `peerId` — the prior fan-to-all leaked one phone's candidates into another phone's connection once >1 device joined the (same-user) room (`electron/remote/desktopPeer.ts`, `Set`→`Map<peerId, pc>`); (b) cap a single `session.input` payload at 64 KiB so an authenticated same-user peer cannot flood the pty with an unbounded write (`electron/remote/remoteMessages.ts`). #70 (signaling-relay replay/freshness) filed as new debt #22 **WONTFIX (accepted risk)**: the relay (`pairingDo.ts:90-94`) is reached only after a valid `session` JWT **and** `claims.userHash === userHashFromPath` (`doProxy.ts` → 401/403), the Durable Object is keyed by `userHash`, and frames are relayed live in-memory between two open WebSockets and never persisted — so a room only ever holds the **same GitHub user's own devices**, there is no stored-message replay surface, and at the WebRTC layer a replayed ICE candidate is idempotent while a stale SDP is rejected by ufrag/m-line checks. Nonce/seq state on an ephemeral same-user relay would add complexity without closing a real exploit path; revisit only if the room threat model widens beyond same-user. #71 (LOW batch: IPC `fromMainFrame` guards, de-hardcode Worker origin) still queued.
