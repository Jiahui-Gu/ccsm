...
| 4 | God-files >500 LOC: `createWindow.ts` (717), `shellRegistry.ts` (686), `log.ts` (637), `sessionCrudSlice.ts` (622), `mobileRemoteServer.ts` (535). **IN PROGRESS**: `createWindow.ts` split (717 → 434 LOC) — CSP/navigation → `csp.ts`, context-menu → `contextMenu.ts`, close-dialog deciders → `closeDialog.ts` (verbatim move). Remaining god-files (`shellRegistry.ts`, `log.ts`, `sessionCrudSlice.ts`, `mobileRemoteServer.ts`) still OPEN. | OPEN | L | HIGH | various |
