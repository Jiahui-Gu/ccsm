...
| 4 | God-files >500 LOC: `createWindow.ts` (717), `shellRegistry.ts` (686), ~~`log.ts` (637)~~ **split → 399 LOC**, `sessionCrudSlice.ts` (622), `mobileRemoteServer.ts` (535). `log.ts` split into `logRuntime.ts`/`logState.ts`/`logFormat.ts`/`logRotation.ts` siblings ([#1430](https://github.com/Jiahui-Gu/ccsm/pull/1430), `ea75fc5`); facade keeps the public logger API. Remaining 4 god-files still OPEN. | OPEN | L | HIGH | various |
