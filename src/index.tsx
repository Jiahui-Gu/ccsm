...
//
// v0.3 wave B1 (Task #627): the renderer-side `installCcsmShim()` is gone
// — the preload bridge in `electron/preload/bridges/ccsmCore.ts` installs
// the full `window.ccsm` surface (6 IPC-only + 25 daemon-backed methods)
// before this script runs. `window.ccsm.window.platform` is a sync
// property pre-resolved from `process.platform` inside preload, so the
// `<DragRegion />` render-time read is always satisfied without any
// boot-time round-trip.
