...
// module-scope `cache` survives the per-case store reset, so a draft cached
// during one case can leak into the next. Exposing `_resetForTests` on the
// window lets the harness scrub the cache without going through DOM input
// events that introduce focus races.
