...
export function createRelayClient(options: RelayClientOptions): RelayClient {
  ...
  async function flushPending(): Promise<void> {
    ...
    while (pendingMessages.length > 0 && socket.readyState === OPEN && peerVerified) {
  ...
  async function handleMessage(current: SocketLike, data: string): Promise<void> {
    let message: unknown;
    try {
    ...
    if (isHello(message)) {
      ...
      const proof = await createHandshakeProof(
        ...
        handshakeTranscript(message, phoneHello),
      );
  ...
  return {
    ...
    send(message) {
      return new Promise<void>((resolve, reject) => {
        pendingMessages.push({ message, resolve, reject });
        void flushPending();
      });
    },
    close() {
      ...
      reconnectTimer = null;
      for (const pending of pendingMessages.splice(0)) pending.reject(new Error('client_closed'));
      if (socket) {
