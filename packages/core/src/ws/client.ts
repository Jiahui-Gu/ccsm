...
//
// Wave-2 T3 (#679): moved verbatim from packages/frontend/src/ws/client.ts
// into the framework-agnostic @ccsm/core package. The ONLY behavioural change
// is `buildWsUrl` no longer reads `window.location` — the caller (web adapter
// or Tauri shell) supplies a `HostBase` describing the http/ws origins. Frame
// decoding (OUTPUT / RESET / EXIT / PAUSE / RESUME) is byte-identical.
//
...
/**
 * Host origin descriptor injected by the adapter (web / Tauri).
 *
 * - `httpBase`: protocol + host (e.g. `http://127.0.0.1:17832` or `https://app.example.com`).
 *   The ws scheme is derived from this (`http` -> `ws`, `https` -> `wss`).
 * - `wsBase` (optional): explicit override when the ws origin differs from
 *   http (e.g. behind a different reverse proxy). Must already include the
 *   `ws://` or `wss://` scheme. When omitted, derived from `httpBase`.
 *
 * Shape kept minimal so a same-origin web caller can produce it from
 * `window.location` and a Tauri caller can produce it from a daemon
 * handshake without dragging URL parsing into core.
 */
...
export interface WsClientOptions {
  sid: string;
  token: string;
  /** Host origins. Required — caller is responsible for the http/ws scheme + host. */
  hostBase: HostBase;
  /**
...
/**
 * Build the ws URL from a caller-supplied `HostBase`.
 *
 * Derivation rules:
 * - If `hostBase.wsBase` is provided, use it as-is for the origin (must
 *   already start with `ws://` or `wss://`).
 * - Otherwise convert `hostBase.httpBase` (`http(s)://host[:port]`) by
...
export function buildWsUrl(
  ...
  const wsOrigin = hostBase.wsBase ?? deriveWsOrigin(hostBase.httpBase);
...
function deriveWsOrigin(httpBase: string): string {
  ...
  // No recognised scheme — pass through; caller supplied something exotic.
  return httpBase;
}
