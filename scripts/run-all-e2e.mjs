...
// `mobile-remote-visual`), each of which owns its own dynamically-ported
// Wrangler dev process (or uses `CCSM_RELAY_URL` when set) — no changes
// needed here to pick them up, they're discovered by the same
// `harness-*.mjs` glob as every other harness below.
//
